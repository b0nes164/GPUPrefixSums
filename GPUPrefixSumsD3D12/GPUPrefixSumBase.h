...
class GPUPrefixSumBase {
   protected:
    ...
    GPUPrefixSumBase(const char* scanName, uint32_t partitionSize, uint32_t maxReadBack)
        : k_scanName(scanName), k_partitionSize(partitionSize), k_maxReadBack(maxReadBack){};

    ~GPUPrefixSumBase(){};

   public:
    virtual void TestExclusiveScanInitOne(uint32_t testSize, bool shouldReadBack,
                                          bool shouldValidate) {
    ...
    virtual void TestInclusiveScanInitOne(uint32_t testSize, bool shouldReadBack,
                                          bool shouldValidate) {
    ...
    virtual void TestInclusiveScanInitRandom(uint32_t testSize, bool shouldReadBack,
                                             bool shouldValidate) {
        if (testSize <= k_maxRandSize) {
   ...
   protected:
    void Initialize() {
        ...
        winrt::check_hresult(m_device->CreateCommandList(0, desc.Type, m_cmdAllocator.get(),
        ...
        winrt::check_hresult(
            m_device->CreateQueryHeap(&queryHeapDesc, IID_PPV_ARGS(m_queryHeap.put())));
    ...
    void InitBuffers(const uint32_t alignedSize, const uint32_t partitions,
        ...
        m_scanInBuffer =
            CreateBuffer(m_device, alignedSize * sizeof(uint32_t), D3D12_HEAP_TYPE_DEFAULT,
        ...
        m_threadBlockReductionBuffer =
            CreateBuffer(m_device, partitions * sizeof(uint32_t), D3D12_HEAP_TYPE_DEFAULT,
                         D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);

        if ((uint32_t)valType > (uint32_t)ValidationType::ONE_EXCLUSIVE) {
            m_scanValidationBuffer = CreateBuffer(
                m_device, alignedSize * sizeof(uint32_t), D3D12_HEAP_TYPE_DEFAULT,
                D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
        } else {
    ...
    void CreateTestInput(uint32_t seed, ValidationType valType) {
        if ((uint32_t)valType > (uint32_t)ValidationType::ONE_EXCLUSIVE) {
            m_initRandom->Dispatch(m_cmdList, m_scanInBuffer->GetGPUVirtualAddress(),
                                   m_scanValidationBuffer->GetGPUVirtualAddress(), m_alignedSize,
                                   seed);
        } else {
            m_initOne->Dispatch(m_cmdList, m_scanInBuffer->GetGPUVirtualAddress(), m_alignedSize);
        }
    ...
    bool ValidateOutput(bool shouldPrint, ValidationType valType) {
        ...
        switch (valType) {
            case ValidationType::ONE_INCLUSIVE:
                m_validateOneInclusive->Dispatch(m_cmdList, m_scanOutBuffer->GetGPUVirtualAddress(),
                                                 m_errorCountBuffer->GetGPUVirtualAddress(),
                                                 m_alignedSize);
                break;
            case ValidationType::ONE_EXCLUSIVE:
                m_validateOneExclusive->Dispatch(m_cmdList, m_scanOutBuffer->GetGPUVirtualAddress(),
                                                 m_errorCountBuffer->GetGPUVirtualAddress(),
                                                 m_alignedSize);
                break;
            case ValidationType::RAND_INCLUSIVE:
                m_validateRandomInclusive->Dispatch(
                    m_cmdList, m_scanOutBuffer->GetGPUVirtualAddress(),
                    m_scanValidationBuffer->GetGPUVirtualAddress(),
                    m_errorCountBuffer->GetGPUVirtualAddress(), m_alignedSize);
                break;
    ...
    void ReadBackOutput() {
        ...
        ReadbackPreBarrier(m_cmdList, m_scanOutBuffer);
        m_cmdList->CopyBufferRegion(m_readBackBuffer.get(), 0, m_scanOutBuffer.get(), 0,
                                    readBackSize * sizeof(uint32_t));
        ReadbackPostBarrier(m_cmdList, m_scanOutBuffer);
        ExecuteCommandList();
    ...
    double TimeScan(uint32_t seed, ValidationType valType) {
        ...
        m_cmdList->ResolveQueryData(m_queryHeap.get(), D3D12_QUERY_TYPE_TIMESTAMP, 0, 2,
