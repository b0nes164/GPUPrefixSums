...
namespace UtilityKernels {
    ...
    class InitOne : public ComputeKernelBase {
       public:
        InitOne(winrt::com_ptr<ID3D12Device> device, const GPUPrefixSums::DeviceInfo& info,
                const std::vector<std::wstring>& compileArguments,
                const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, info, shaderPath, L"InitOne", compileArguments,
                                CreateRootParameters()) {}

        void Dispatch(winrt::com_ptr<ID3D12GraphicsCommandList> cmdList,
                      const D3D12_GPU_VIRTUAL_ADDRESS& scanBuffer, const uint32_t& size) {
    ...
    class InitRandom : public ComputeKernelBase {
       public:
        InitRandom(winrt::com_ptr<ID3D12Device> device, const GPUPrefixSums::DeviceInfo& info,
                   const std::vector<std::wstring>& compileArguments,
                   const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, info, shaderPath, L"InitRandom", compileArguments,
                                CreateRootParameters()) {}

        void Dispatch(winrt::com_ptr<ID3D12GraphicsCommandList> cmdList,
                      const D3D12_GPU_VIRTUAL_ADDRESS& scanBuffer,
                      const D3D12_GPU_VIRTUAL_ADDRESS& scanValidationBuffer, const uint32_t& size,
                      const uint32_t& seed) {
    ...
    class ClearErrorCount : ComputeKernelBase {
       public:
        ClearErrorCount(winrt::com_ptr<ID3D12Device> device, const GPUPrefixSums::DeviceInfo& info,
                        const std::vector<std::wstring>& compileArguments,
                        const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, info, shaderPath, L"ClearErrorCount", compileArguments,
                                CreateRootParameters()) {}
    ...
    class ValidateOneInclusive : public ComputeKernelBase {
       public:
        ValidateOneInclusive(winrt::com_ptr<ID3D12Device> device,
                             const GPUPrefixSums::DeviceInfo& info,
                             const std::vector<std::wstring>& compileArguments,
                             const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, info, shaderPath, L"ValidateOneInclusive", compileArguments,
                                CreateRootParameters()) {}

        void Dispatch(winrt::com_ptr<ID3D12GraphicsCommandList> cmdList,
                      const D3D12_GPU_VIRTUAL_ADDRESS& scanBuffer,
                      const D3D12_GPU_VIRTUAL_ADDRESS& errorCount, const uint32_t& size) {
            std::array<uint32_t, 4> t = {size, 0, 0, 0};
            SetPipelineState(cmdList);
            cmdList->SetComputeRoot32BitConstants(0, 4, t.data(), 0);
            cmdList->SetComputeRootUnorderedAccessView(1, scanBuffer);
    ...
    class ValidateOneExclusive : ComputeKernelBase {
       public:
        ValidateOneExclusive(winrt::com_ptr<ID3D12Device> device,
                             const GPUPrefixSums::DeviceInfo& info,
                             const std::vector<std::wstring>& compileArguments,
                             const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, info, shaderPath, L"ValidateOneExclusive", compileArguments,
                                CreateRootParameters()) {}
    ...
    class ValidateRandomExclusive : public ComputeKernelBase {
       public:
        ValidateRandomExclusive(winrt::com_ptr<ID3D12Device> device,
                                const GPUPrefixSums::DeviceInfo& info,
                                const std::vector<std::wstring>& compileArguments,
                                const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, info, shaderPath, L"ValidateRandomExclusive",
    ...
    class ValidateRandomInclusive : ComputeKernelBase {
       public:
        ValidateRandomInclusive(winrt::com_ptr<ID3D12Device> device,
                                const GPUPrefixSums::DeviceInfo& info,
                                const std::vector<std::wstring>& compileArguments,
                                const std::filesystem::path& shaderPath)
            : ComputeKernelBase(device, info, shaderPath, L"ValidateRandomInclusive",
                                compileArguments, CreateRootParameters()) {}
