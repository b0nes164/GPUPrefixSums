...
GPUPrefixSums::DeviceInfo GetDeviceInfo(ID3D12Device* device) {
    ...
    bool isWarpDevice =
        ((adapterDesc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE) == DXGI_ADAPTER_FLAG_SOFTWARE) ||
    ...
    winrt::check_hresult(
        device->CheckFeatureSupport(D3D12_FEATURE_SHADER_MODEL, &model, sizeof(model)));
    ...
    std::cout << "Supports Wave Intrinsics:              "
              << (devInfo.SupportsWaveIntrinsics ? "Yes" : "No") << "\n";
    std::cout << "Supports ReduceThenScan:               "
              << (devInfo.SupportsReduceThenScan ? "Yes" : "No") << "\n";
#endif
...
winrt::com_ptr<ID3D12Device> InitDevice() {
    ...
    winrt::check_hresult(
        D3D12CreateDevice(nullptr, D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(device.put())));
    return device;
}
...
winrt::com_ptr<ID3D12Device> InitDeviceWarp() {
    ...
    winrt::check_hresult(
        D3D12CreateDevice(adapter.get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(device.put())));
    return device;
}
...
int main() {
    ...
    ChainedScanDecoupledLookbackDecoupledFallback* csdldf =
        new ChainedScanDecoupledLookbackDecoupledFallback(device, deviceInfo);
    csdldf->TestAll();
    csdldf->BatchTimingInclusiveInitOne(1 << 28, 100);
    csdldf->~ChainedScanDecoupledLookbackDecoupledFallback();

    ChainedScanDecoupledLookback* csdl = new ChainedScanDecoupledLookback(device, deviceInfo);
    csdl->TestAll();
