...
namespace SurveyKernels {
    class SerialInclusive : public SurveyKernelBase {
       public:
        SerialInclusive(winrt::com_ptr<ID3D12Device> device, const GPUPrefixSums::DeviceInfo& info,
                        const std::vector<std::wstring>& compileArguments,
                        const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath, L"SerialInclusive") {}
    };

    class SerialExclusive : public SurveyKernelBase {
       public:
        SerialExclusive(winrt::com_ptr<ID3D12Device> device, const GPUPrefixSums::DeviceInfo& info,
                        const std::vector<std::wstring>& compileArguments,
                        const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath, L"SerialExclusive") {}
    };

    class WaveKoggeStoneInclusive : public SurveyKernelBase {
       public:
        WaveKoggeStoneInclusive(winrt::com_ptr<ID3D12Device> device,
                                const GPUPrefixSums::DeviceInfo& info,
                                const std::vector<std::wstring>& compileArguments,
                                const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
                               L"WaveKoggeStoneInclusive") {}
    };

    class WaveKoggeStoneExclusive : public SurveyKernelBase {
       public:
        WaveKoggeStoneExclusive(winrt::com_ptr<ID3D12Device> device,
                                const GPUPrefixSums::DeviceInfo& info,
                                const std::vector<std::wstring>& compileArguments,
                                const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class WaveKoggeStoneShuffleInclusive : public SurveyKernelBase {
       public:
        WaveKoggeStoneShuffleInclusive(winrt::com_ptr<ID3D12Device> device,
                                       const GPUPrefixSums::DeviceInfo& info,
                                       const std::vector<std::wstring>& compileArguments,
                                       const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class WaveKoggeStoneShuffleExclusive : public SurveyKernelBase {
       public:
        WaveKoggeStoneShuffleExclusive(winrt::com_ptr<ID3D12Device> device,
                                       const GPUPrefixSums::DeviceInfo& info,
                                       const std::vector<std::wstring>& compileArguments,
                                       const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class WaveKoggeStoneIntrinsicInclusive : public SurveyKernelBase {
       public:
        WaveKoggeStoneIntrinsicInclusive(winrt::com_ptr<ID3D12Device> device,
                                         const GPUPrefixSums::DeviceInfo& info,
                                         const std::vector<std::wstring>& compileArguments,
                                         const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class WaveKoggeStoneIntrinsicExclusive : public SurveyKernelBase {
       public:
        WaveKoggeStoneIntrinsicExclusive(winrt::com_ptr<ID3D12Device> device,
                                         const GPUPrefixSums::DeviceInfo& info,
                                         const std::vector<std::wstring>& compileArguments,
                                         const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class WaveRakingReduceInclusive : public SurveyKernelBase {
       public:
        WaveRakingReduceInclusive(winrt::com_ptr<ID3D12Device> device,
                                  const GPUPrefixSums::DeviceInfo& info,
                                  const std::vector<std::wstring>& compileArguments,
                                  const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class WaveRakingReduceExclusive : public SurveyKernelBase {
       public:
        WaveRakingReduceExclusive(winrt::com_ptr<ID3D12Device> device,
                                  const GPUPrefixSums::DeviceInfo& info,
                                  const std::vector<std::wstring>& compileArguments,
                                  const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockKoggeStoneInclusive : public SurveyKernelBase {
       public:
        BlockKoggeStoneInclusive(winrt::com_ptr<ID3D12Device> device,
                                 const GPUPrefixSums::DeviceInfo& info,
                                 const std::vector<std::wstring>& compileArguments,
                                 const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockKoggeStoneExclusive : public SurveyKernelBase {
       public:
        BlockKoggeStoneExclusive(winrt::com_ptr<ID3D12Device> device,
                                 const GPUPrefixSums::DeviceInfo& info,
                                 const std::vector<std::wstring>& compileArguments,
                                 const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockSklanskyInclusive : public SurveyKernelBase {
       public:
        BlockSklanskyInclusive(winrt::com_ptr<ID3D12Device> device,
                               const GPUPrefixSums::DeviceInfo& info,
                               const std::vector<std::wstring>& compileArguments,
                               const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockSklanskyExclusive : public SurveyKernelBase {
       public:
        BlockSklanskyExclusive(winrt::com_ptr<ID3D12Device> device,
                               const GPUPrefixSums::DeviceInfo& info,
                               const std::vector<std::wstring>& compileArguments,
                               const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockBrentKungBlellochInclusive : public SurveyKernelBase {
       public:
        BlockBrentKungBlellochInclusive(winrt::com_ptr<ID3D12Device> device,
                                        const GPUPrefixSums::DeviceInfo& info,
                                        const std::vector<std::wstring>& compileArguments,
                                        const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockBrentKungBlellochExclusive : public SurveyKernelBase {
       public:
        BlockBrentKungBlellochExclusive(winrt::com_ptr<ID3D12Device> device,
                                        const GPUPrefixSums::DeviceInfo& info,
                                        const std::vector<std::wstring>& compileArguments,
                                        const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockReduceScanInclusive : public SurveyKernelBase {
       public:
        BlockReduceScanInclusive(winrt::com_ptr<ID3D12Device> device,
                                 const GPUPrefixSums::DeviceInfo& info,
                                 const std::vector<std::wstring>& compileArguments,
                                 const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockReduceScanExclusive : public SurveyKernelBase {
       public:
        BlockReduceScanExclusive(winrt::com_ptr<ID3D12Device> device,
                                 const GPUPrefixSums::DeviceInfo& info,
                                 const std::vector<std::wstring>& compileArguments,
                                 const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockBrentKungIntrinsicInclusive : public SurveyKernelBase {
       public:
        BlockBrentKungIntrinsicInclusive(winrt::com_ptr<ID3D12Device> device,
                                         const GPUPrefixSums::DeviceInfo& info,
                                         const std::vector<std::wstring>& compileArguments,
                                         const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockBrentKungIntrinsicExclusive : public SurveyKernelBase {
       public:
        BlockBrentKungIntrinsicExclusive(winrt::com_ptr<ID3D12Device> device,
                                         const GPUPrefixSums::DeviceInfo& info,
                                         const std::vector<std::wstring>& compileArguments,
                                         const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockBrentKungFusedIntrinsicInclusive : public SurveyKernelBase {
       public:
        BlockBrentKungFusedIntrinsicInclusive(winrt::com_ptr<ID3D12Device> device,
                                              const GPUPrefixSums::DeviceInfo& info,
                                              const std::vector<std::wstring>& compileArguments,
                                              const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockSklanskyIntrinsicInclusive : public SurveyKernelBase {
       public:
        BlockSklanskyIntrinsicInclusive(winrt::com_ptr<ID3D12Device> device,
                                        const GPUPrefixSums::DeviceInfo& info,
                                        const std::vector<std::wstring>& compileArguments,
                                        const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockSklanskyIntrinsicInclusiveAlt : public SurveyKernelBase {
       public:
        BlockSklanskyIntrinsicInclusiveAlt(winrt::com_ptr<ID3D12Device> device,
                                           const GPUPrefixSums::DeviceInfo& info,
                                           const std::vector<std::wstring>& compileArguments,
                                           const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockSklanskyIntrinsicExclusive : public SurveyKernelBase {
       public:
        BlockSklanskyIntrinsicExclusive(winrt::com_ptr<ID3D12Device> device,
                                        const GPUPrefixSums::DeviceInfo& info,
                                        const std::vector<std::wstring>& compileArguments,
                                        const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockRakingReduceIntrinsicInclusive : public SurveyKernelBase {
       public:
        BlockRakingReduceIntrinsicInclusive(winrt::com_ptr<ID3D12Device> device,
                                            const GPUPrefixSums::DeviceInfo& info,
                                            const std::vector<std::wstring>& compileArguments,
                                            const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class BlockRakingReduceIntrinsicExclusive : public SurveyKernelBase {
       public:
        BlockRakingReduceIntrinsicExclusive(winrt::com_ptr<ID3D12Device> device,
                                            const GPUPrefixSums::DeviceInfo& info,
                                            const std::vector<std::wstring>& compileArguments,
                                            const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class SharedBrentKungFusedIntrinsicInclusive : public SurveyKernelBase {
       public:
        SharedBrentKungFusedIntrinsicInclusive(winrt::com_ptr<ID3D12Device> device,
                                               const GPUPrefixSums::DeviceInfo& info,
                                               const std::vector<std::wstring>& compileArguments,
                                               const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class SharedBrentKungFusedIntrinsicExclusive : public SurveyKernelBase {
       public:
        SharedBrentKungFusedIntrinsicExclusive(winrt::com_ptr<ID3D12Device> device,
                                               const GPUPrefixSums::DeviceInfo& info,
                                               const std::vector<std::wstring>& compileArguments,
                                               const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class SharedRakingReduceIntrinsicInclusive : public SurveyKernelBase {
       public:
        SharedRakingReduceIntrinsicInclusive(winrt::com_ptr<ID3D12Device> device,
                                             const GPUPrefixSums::DeviceInfo& info,
                                             const std::vector<std::wstring>& compileArguments,
                                             const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class SharedRakingReduceIntrinsicExclusive : public SurveyKernelBase {
       public:
        SharedRakingReduceIntrinsicExclusive(winrt::com_ptr<ID3D12Device> device,
                                             const GPUPrefixSums::DeviceInfo& info,
                                             const std::vector<std::wstring>& compileArguments,
                                             const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class TrueBlockInclusiveScan : public SurveyKernelBase {
       public:
        TrueBlockInclusiveScan(winrt::com_ptr<ID3D12Device> device,
                               const GPUPrefixSums::DeviceInfo& info,
                               const std::vector<std::wstring>& compileArguments,
                               const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
    ...
    class TrueBlockExclusiveScan : public SurveyKernelBase {
       public:
        TrueBlockExclusiveScan(winrt::com_ptr<ID3D12Device> device,
                               const GPUPrefixSums::DeviceInfo& info,
                               const std::vector<std::wstring>& compileArguments,
                               const std::filesystem::path& shaderPath)
            : SurveyKernelBase(device, info, compileArguments, shaderPath,
