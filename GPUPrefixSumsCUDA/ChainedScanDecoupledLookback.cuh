...
namespace ChainedScanDecoupledLookback {
    ...
    __global__ void CSDLInclusive(uint32_t* scan, volatile uint32_t* threadBlockReduction,
        ...
        uint4 tScan[PER_THREAD];
        const uint32_t offset = WARP_INDEX * LANE_COUNT * PER_THREAD + partitionIndex * PART_VEC_SIZE;
        if (partitionIndex < gridDim.x - 1) {
        ...
        // You can try using this instead of the warp method!
        /*if (partitionIndex && !threadIdx.x) {
            LookbackSingle(partitionIndex, s_warpReduction[WARPS - 1], s_broadcast,
                           threadBlockReduction);
        }*/
        __syncthreads();
